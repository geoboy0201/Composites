function [PxMaxTsaiWu]=compositePlateFailureTsaiWu(laminate,L,W)
...
%     PxMax(2,i)=sol(2);
    ...
    C=[C1,C2,C3];
    sol=roots(C);
    PxMax(i-1)=sol(sol>0); %For Exam 3
%     PxMax(1,i-1)=sol(1);
%     PxMax(2,i-1)=sol(2);
end

[PxMaxTsaiWu,I]=min(abs(PxMax),[],'all','linear');
PxMaxTsaiWu=PxMax(I);
